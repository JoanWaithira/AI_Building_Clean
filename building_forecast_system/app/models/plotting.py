...
# non-GUI backend for Windows / schedulers
matplotlib.use("Agg")
...
def _bar_colors(df: pd.DataFrame, selection_metric: str):
    """
    Highlight the best model according to the chosen selection metric.
    """
...
def plot_model_comparison_for_circuit(results_df: pd.DataFrame, circuit_id: str, horizon_type: str = "short"):
    """
    Old simple single-panel chart: Test RMSE only.
    Still useful if you want a quick chart.
    """
...
def plot_complete_model_performance_for_circuit(results_df: pd.DataFrame, circuit_id: str, horizon_type: str = "short"):
    """
    Multi-panel chart like the example:
    - Val MAE
    - Val RMSE
    - Test MAE
    - Test RMSE

    Winner is selected by lowest validation RMSE and highlighted in all panels.
    """
    df = _prepare_circuit_df(results_df, circuit_id, horizon_type)
...
def plot_inference_forecast(history_df: pd.DataFrame, forecast_df: pd.DataFrame, circuit_id: str, horizon_type: str = "short"):
    """
    Plot recent history and the generated forecast together.
    """
    ...
    plt.title(f"{circuit_id} - {horizon_type} inference forecast")
    plt.xlabel("Time")
    plt.ylabel("Value")
    plt.legend()
    plt.tight_layout()
