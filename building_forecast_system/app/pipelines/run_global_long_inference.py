...
OUTPUT_FILE = FORECASTS_DIR / "global_long_hourly_forecast.csv"
...
def add_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for lag in [1, 3, 6, 12, 24, 48, 72, 96, 168]:
...
def run():
    run_id = start_pipeline_run("run_global_long_inference")
    print("\nRunning GLOBAL long-term hourly inference")

    try:
        if not MODEL_FILE.exists():
            print(f"Long-term model not found: {MODEL_FILE}")
            finish_pipeline_run(run_id, status="failed", error_message=f"Long-term model not found: {MODEL_FILE}")
            return
        ...
        horizon = int(bundle.get("forecast_horizon_steps", 48))
        ...
        if history_df.empty or len(history_df) < 200:
            message = "Not enough history for global long-term inference."
            print(message)
            ...
            predictions.append(
                {
                    ...
                    "model_version": "global_best_long_hourly_model",
                    "resolution": "1h",
                }
            )
        ...
        print(f"Saved global long-term hourly forecast -> {OUTPUT_FILE}")
        ...
        try:
            ...
            print(f"Wrote {len(db_forecast_df)} global long-term forecast rows to database")
        except Exception as exc:
            print(f"Database write failed for global long forecast: {exc}")
        ...
        print(f"Saved unified global long-term forecast -> {persist_result['csv_path']}")
        if persist_result["db_written"]:
            print(f"Updated database table -> {persist_result['table_name']}")

        try:
            history_for_plot = history_df.tail(168).copy()
