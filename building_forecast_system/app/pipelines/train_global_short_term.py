...
from app.pipelines.global_features_hourly import build_global_short_term_hourly_dataset
...
def run():
    ...
    print("Training global short-term hourly models")

    df = pd.read_csv(GLOBAL_FILE)
    df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)

    dataset, feature_cols = build_global_short_term_hourly_dataset(df)
    ...
    for model_name, model in candidate_models.items():
        print(f"Training global short-term hourly model: {model_name}")
    ...
    if best_model_bundle is not None:
        ...
        print(f"Saved global best model: {model_file}")
    ...
    metrics_file = METRICS_DIR / "global_short_hourly_results.csv"
    results_df.to_csv(metrics_file, index=False)

    print(f"Saved global metrics: {metrics_file}")
    ...
    plot_best_model_counts(results_df, horizon_type="short_hourly")

    print("Finished global short-term hourly training")
