"""
Database-based forecast provider using SQLAlchemy.
"""
...
class DbForecastProvider(ForecastProvider):
    """Provides forecasts from PostgreSQL database."""

    def __init__(self, db_session: Session):
        """
        Initialize DB provider with active session.
    ...
    async def get_forecasts(
        ...
        try:
            ...
            if not rows:
                logger.debug(f"No rows from DB for scope={scope}, product={product}")
                return []
